...
def farm_energy_production_calculation():
    ...
    farm_energy_production_data = pd.read_csv(os.path.join(dirname, r"netlogo\farm-energy-production.csv"), delimiter="\t", header=None)
...
def energy_net_income_calculation():
    ...
    energy_net_income_data = pd.read_csv(os.path.join(dirname, r"netlogo\energy-net-income.csv"), delimiter="\t", header=None)
    ...
    energyproductiontitle = f'Farm Energy Production - Start Year: {start_year}'

    # Create the dictionary conditionally for each column
    temp_dict = {'Year': df['year'].values.tolist()}

    columns_to_include = ['Wind','Solar','US$0']
    for column in columns_to_include:
        if not (df[column] == 0).all():  # Check if all values in the column are not zero
            temp_dict[column] = df[column].values.tolist()
    temp_dict['energyproductiontitle'] = energyproductiontitle
